...
OptFlowFrame::OptFlowFrame()
{
    ...
    this->max_iterations_ = 20;
    this->min_eigen_threshold_ = 0.01f;
    ...
    if(this->frames_.empty() || this->change_directory_)
    {
        ...
        if(exists(frames_dir))
        {
            ...
            for(int i=0; i < num_frames_set.size(); i++ )
            {
                ...
                image_temp = imread(complete_path, IMREAD_COLOR);   // Read the file
...
void OptFlowFrame::write_first_frame_with_optical_flow(bool show_output)
{
    ...
    cvtColor(first_frame_temp, prev_gray, COLOR_BGR2GRAY);
    ...
    for(int j = 1; j < this->frames_.size(); j++)
    {
        // Get the next frame
        ...
        if(cpt % 15 == 0)
        {
            goodFeaturesToTrack(gray, points[0], this->max_features_, this->quality_level_, this->min_distance_, Mat(), this->block_size_, this->block_size_, this->use_harris_detector_, 0.04);
            // OLD VERSION
            //goodFeaturesToTrack(prev_gray, points[0], this->max_features_, this->quality_level_, this->min_distance_, Mat(), this->block_size_, this->use_harris_detector_, 0.04);
        }
...
void OptFlowFrame::write_vector_video(bool write_json_vector, bool show_output, int fps)
{
    ...
    VideoWriter writer;
    int codec = writer.fourcc('M', 'J', 'P', 'G');  // select desired codec (must be available at runtime)
    ...
    int cpt = 1;
    //TicToc to get the time of execution
    TicToc time_exec;
